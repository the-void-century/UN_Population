        ...
        for population in population_reader:
            ...
            if population["Region"] in ASEAN_COUNTRIES:
                if population["Year"] not in ASEAN_BY_YEAR:
                    ...
                    if population["Region"] not in ASEAN_BY_YEAR[population["Year"]]:
...
def asean_population_over_year():
    ...
    for years in ASEAN_BY_YEAR:
        country=list(ASEAN_BY_YEAR[years].keys())
        population=list(ASEAN_BY_YEAR[years].values())
        country,population=zip(*sorted(zip(country,population)))
        plt.bar(country,population,width=0.2)
    plt.xticks(Years,rotation=45)
    plt.legend(ASEAN_COUNTRIES)
    plt.show()
